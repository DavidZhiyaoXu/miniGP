...
def plot_diffeomorphism_1D(X_test: jnp.ndarray, kernel, kernel_params) -> None:
    """
    Draws the output of the R^d to R^d diffeomorphism from a grid of points X_test.
    
    Parameters:
    - X_test: jnp.ndarray, grid of points in R^d, shape (n_points, d).
    - kernel: kernel function with a neural network component that acts as a diffeomorphism.
    - kernel_params: parameters for the kernel, including the neural network parameters.
    """
    # Apply the neural network transformation to get the transformed points
